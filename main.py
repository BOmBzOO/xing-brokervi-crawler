...
class COLLECT_DATA(QtWidgets.QMainWindow):
    ...
    def UpdateWindowTitle(self):
        if self.showQsize:
            ...
            text = f'BROKER & VI INFO Crawler - Qsize : 코스피 [{kospiQ_size}] | 코스닥 [{kosdaqQ_size} | VI [{viQ.qsize()}]]'
            self.setWindowTitle(text)
        elif self.windowTitle() != 'BROKER & VI INFO Crawler':
    ...
    def closeEvent(self, a):
        buttonReply = QtWidgets.QMessageBox.question(
            self, "프로그램 종료", "BROKER & VI INFO 프로그램을 종료합니다.",
...
if __name__ == "__main__":
    ...
    receiver_proc = []
    receiver_p = Process(target=Receiver, args=(kospi_qlist, kosdaq_qlist, viQ, windowQ,))
    receiver_proc.append(receiver_p)

    collector_kospi_proc=[]
    for idx, queue in enumerate(kospi_qlist):
        kospi_p = Process(target=Collecter, args=(f'브로커코스피_{idx}', kospi_qlist, writer_qlist, windowQ,))
        collector_kospi_proc.append(kospi_p)

    collector_kosdaq_proc=[]
    for idx, queue in enumerate(kosdaq_qlist):
        kosdaq_p = Process(target=Collecter, args=(f'브로커코스닥_{idx}', kosdaq_qlist, writer_qlist, windowQ,))
        collector_kosdaq_proc.append(kosdaq_p)

    collector_vi_proc = [Process(target=Collecter, args=(f'VI발동해제_0', viQ, writer_qlist, windowQ,))]
    ...
    processes = receiver_proc + collector_kospi_proc + collector_kosdaq_proc + collector_vi_proc + writer_proc
